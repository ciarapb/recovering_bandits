...
# -*- coding: utf-8 -*-
"""
K=10, medium GP setup
squared exponential kernel with l=2
"""
...
class Arm:
    def __init__(self, samples, index, zinit, noisevar):
        ...
        self.res = 0.0
        self.model = None
        self.kernel = GPy.kern.RBF(input_dim=1, variance=1., lengthscale=2.)

    # associate a reward function to each arm
    def rewfunct(self, z):
        zint = int(z)
        rew = self.samples[zint]
        return rew

    # define function to sample arm
    def sample(self, z):
...
kern = GPy.kern.RBF(input_dim=1, variance=1., lengthscale=2.)
